...
expName = 'stim-sequence'  # from the Builder filename that created this script
expInfo = {
...
# --- Setup the Window ---
win = visual.Window(
    size=[2056, 1329], fullscr=True, screen=0, 
...
while continueRoutine and routineTimer.getTime() < 1.5:
    ...
    if welcometext.status == STARTED:
        ...
        if tThisFlipGlobal > welcometext.tStartRefresh + 1.5-frameTolerance:
...
trials = data.TrialHandler(nReps=1.0, method='random', 
    ...
    trialList=data.importConditions('stim_list_2.xlsx'),
    seed=42, name='trials')
...
for thisTrial in trials:
    ...
    while continueRoutine and routineTimer.getTime() < 3.0:
        ...
        if sounds.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > sounds.tStartRefresh + 3-frameTolerance:
                # keep track of stop time/frame for later
